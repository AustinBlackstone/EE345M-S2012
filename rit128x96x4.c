...
#include <string.h> //OLED_SplitScreen
//global to hold scroll lines
char scrollLines[8][21];
...
//	int stringLength;
	...
	if((strlen(valueToString)+strlen(string))> 20){
		...
		for(y=strlen(string);y<strlen(string)+strlen(valueToString);y++){	//concattenate bottom line : number part
			scrollLines[0][y]=valueToString[y-strlen(string)];
		}	
	}
	for(x=0;x<8;x++){	//print it all out
		RIT128x96x4StringDraw(scrollLines[x] , 0, (98-(x*12)), 15);	//98 is used to make it print bottom up, remote the '98-' and it will print top down	
